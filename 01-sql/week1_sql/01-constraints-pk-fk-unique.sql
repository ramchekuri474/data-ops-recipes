...
CREATE TABLE customers (
  ...
  email       VARCHAR(255) UNIQUE,             -- Unique Key: ensures email is unique (can be NULL in some DBs)
...
-- 5) Example: UNIQUE constraint violation (should fail)
-- Try inserting a customer with same email:
-- INSERT INTO customers (customer_id, email, full_name)
-- VALUES (3, 'alice@example.com', 'Alice Duplicate');

-- 6) Example: FK violation (should fail)
...
-- 8) If FK constraints are NOT enforced (common in lakehouse),
